{
	// MM 24/01/13 created to compare MCAL triggers to IPN catalog to update MCAL catalog paper and AGILE detections in IPN list
	// based on compare_GBM_MCAL_triggers.C
	...
	while (!fin.eof()) {
		...
		if (atrg.t0 >= 143085852. && atrg.t0 <= 205770462. && strcmp(atrg.type, "BOOST")) {
	...
	char grbdate[20];
	char grbtime[20];
	int yy, mm, dd, hh, min, sec, nsec, sod;
	char month[5];
	...
	ifstream fagileipn;
	fagileipn.open("/home/mcal/data_analysis/grb/ipngrb.txt", ifstream::in);
	int nipn = 0;
	nsec = 0;
	double ttipn[10000];
	while (fagileipn >> dd >> month >> yy >> grbtime >> sod) {
		...
		if (inlist) {
			if (ttipn[nipn] - t03908[pos] <= tthr) { 
				printf("%02d%02d%02d %s %06d %d %20.6f\n", yy, mm, dd, grbtime, cont[pos], trg[pos], t03908[pos]);
			} else if (t03908[pos+1] - ttipn[nipn] <= tthr) { 
				printf("%02d%02d%02d %s %06d %d %20.6f\n", yy, mm, dd, grbtime, cont[pos+1], trg[pos+1], t03908[pos+1]);
			}
		}
