...
// ottimizzazione AC, setting width e delay Dicembre '06
char datadir[]="/data1/AGILE_MCAL_AIV_Data/CGS_Tortona/test_AC_soglie_061206/rootfiles/";
char f_ACoff[]="6746_E.root";
char f_AC3131[]="6772_E.root";
char f_AC3116[]="6797_E.root";
char f_ACtest1[]="11828_E.root";
//char f_AC3131top[]="5901_E.root";

char filepath[100];

sprintf(filepath, "%s%s", datadir, f_ACoff);
TFile fi(filepath) ;
TPad *pi;
...
sprintf(filepath, "%s%s", datadir, f_AC3116);
TFile f3(filepath) ;
TPad *p3;
...
sprintf(filepath, "%s%s", datadir, f_ACtest1);
TFile f4(filepath) ;
TPad *p4;
...
leg->AddEntry(hi, "run  6746 - AC off", "l");
leg->AddEntry(he, "run  6772 - AC on, top on, w=31 d=31", "l");
leg->AddEntry(h3, "run  6797 - AC on, top on, w=16 d=31", "l");
leg->AddEntry(h4, "run 11828 - AC on, top on, w=16 d=31", "l");
//leg->AddEntry(h5, "run 5901 - AC on, top on, w=d=31", "l");
leg->Draw();

cout << "run  6746 integral for E>3 MeV " << hi->Integral(4, 100) << "  -  Total rate " << hi->Integral() << " evt/s" << endl;
cout << "run  6772 integral for E>3 MeV " << he->Integral(4, 100) << "  -  Total rate " << he->Integral() << " evt/s" <<  endl;
cout << "run  6797 integral for E>3 MeV " << h3->Integral(4, 100) << "  -  Total rate " << h3->Integral() << " evt/s" <<  endl;
cout << "run 11828 integral for E>3 MeV " << h4->Integral(4, 100) << "  -  Total rate " << h4->Integral() << " evt/s" <<  endl;
//cout << "run 5901 integral for E>3 MeV " << h5->Integral(3, 100) << endl;

float cc1, cc2,cc3,cc4;
cc1=cc2=cc3=cc4=0.;
for (int i=4; i<100; i++) {
	cc1 += hi->GetBinContent(i);
	cc2 += he->GetBinContent(i);
	cc3 += h3->GetBinContent(i);
	cc4 += h4->GetBinContent(i);
	cout << hi->GetBinContent(i) << "  " << he->GetBinContent(i) << "  " << h3->GetBinContent(i) << "  " << h4->GetBinContent(i) << endl;
}
cout << endl << cc1 << "  " << cc2 << "  " << cc3 << "  " << cc4 << endl;
