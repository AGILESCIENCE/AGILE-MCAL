{
	// MM 26/04/11 version _8 based on version _7. Includes analysis of offline triggers.
	// MM 09/03/11 version _7 based on version _6. Dedicated to the Mar2009 - Feb2011 dataset for the catalog
	
	// MM 21/04/10 output t0 list for off-line trigger search
	// MM 08/02/10 added zoom over South-East Asia
	// MM 09/12/09 modified print for use with makespectra (see changelog) print only: contact t0 dt
	// MM 23/10/09 added zoom over Africa
...
// 	mcalstyle->SetLabelSize(0.05, "Z");
	...
	Trigger atrg;
	 ...
	 // /data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_Jun08-Jan10_E-greater-40MeV.txt    // >40MeV dataset for Tavani eta al. 2010 PRL paper
	 // tvsb.ReadFile("/data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_Mar2009-Feb2011.txt", 
	  //tvsb.ReadFile("/data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_Mar09-Feb11_E-greater-30MeV.txt",
	   //tvsb.ReadFile("/data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_Mar2009-Feb2011.txt", 
	   //tvsb.ReadFile("/data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_Mar09-Feb11_E-greater-30MeV.txt",
	   tvsb.ReadFile("/data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_Mar2009-Feb2011.txt",  "runid/I:trg/I:t0/D:tstop/D:ttot/F:date/C:time/C:mint3908/F:maxt3908/F:type/C:sa/I:lng/I:subms/I:flg01ms/I:flg16ms/I:nrm/I:rmid/I:sitid/I:ber/I:bet/I:lon/F:lat/F:h/F:dt/F:ntrg/I:mint/F:dt0/F:tgrbini/F:t90/F:tbin/F:cbkg/F:counts/F:cpeak/F:avgE/F:nEbins/I:c0/F:c1/F:c2/F:c3/F:b0/F:b1/F:b2/F:b3/F:x1/I:x2/I:x3/I:x4/I:x5/I:x6/I:x7/I:x8/I:x9/I:x10/I:x11/I:x12/I:x13/I:x14/I:x15/I:z1/I:z2/I:z3/I:z4/I:z5/I:z6/I:z7/I:z8/I:z9/I:z10/I:z11/I:z12/I:z13/I:z14/I:z15/I:cub1/I:cub2/I:cub3/I:cub4/I:cub5/I:cub6/I:cub7/I:cub8/I");
	...
	int lonbin = 72; // 12 o 36 o 72 o 144
	...
	TH1F *ht90 = new TH1F("ht90", "ht90", 400, 0., 0.1);	//200
	TH1F *hcpeak = new TH1F("hcpeak", "hcpeak", 100, 0, 100);
	TH1F *hcounts = new TH1F("hcounts", "hcounts", 500, 0, 500);
	...
	TH1F *hdeltat = new TH1F("hdeltat","hdeltat", nbins, abin);	
	...
	// read MCAL offline triggers and fill histo
	
	TH1F *ht90off = new TH1F("ht90off", "ht90off", 1000, 0., 0.1);	//200
	TH1F *hcountsoff = new TH1F("hcountsoff", "hcountsoff", 500, 0, 500);
	ifstream intrg;
	intrg.open("/home/mcal/data_analysis/VSB/offline_trigger_search/trigger_search_2/orbits_009564-19872_Feb2011/cut3_no-SAA_expo.dat");
	...
	for (int i=0; i<nentries; i++) {
		...
		// if ((year == 2009 && month>=4) || year > 2009) hm->Fill(month);
		hm->Fill(month);
	...
	*/		
		...
		if (1) { // year>=2009 && month>=3
			...
			// fill waiting time histo
			if (t0prev) {
				hdeltat->Fill((atrg.t0-t0prev)/60.);
			}
			t0prev = atrg.t0;
			...
			// output t0 list for off-line trigger search
			if (0) {
				double peakdt = (fmod(atrg.t0,1.) + atrg.tgrbini);
				if (peakdt>=0. && peakdt<1.) {
					printf("%6d \t%20.6f\n",atrg.runid, atrg.t0 + atrg.tgrbini);
				} else cout << "check trigger " << atrg.runid << "-" << atrg.trg << endl;
			}
			...
			// format for table in Tavani TGF PRL paper 
			if (0) {
				double peakdt = (fmod(atrg.t0,1.) + atrg.tgrbini);
				if (peakdt>=0. && peakdt<1.) {
					printf("%6d-%d & \t%s & \t%s.%06d & \t%10.2f & \t%10.2f & \t%10.1f & \t%10.0f &\n",atrg.runid, atrg.trg, atrg.date, atrg.time, (int) (peakdt*1.E6), atrg.lon, atrg.lat, 1000.*atrg.t90, atrg.counts);
				} else cout << "check trigger " << atrg.runid << "-" << atrg.trg << endl;
...
/*	ht90->Scale(1./nGRB);
	...
	TCanvas ccomp("ccomp", "ccomp", 800, 600);
	ccomp.Divide(1, 2);
	ccomp.SetTicks(1,1);
	...
	hlonoff->Sumw2();
	hlonoff->Scale(1./noff);
	hlonoff->SetMarkerStyle(20);
	hlonoff->Draw("E1PSAME");
...
/*	c1.cd(1);
	...
	ht90->GetYaxis()->SetTitle("Normalized counts");
	...
	//ht90->Scale(1./nentries);
	ht90->Draw();
	
	//ht90off->Scale(1./noff);
...
//	hcpeakBUS->Draw("SAME");
	...
	TCanvas cdeltat("cdeltat", "cdeltat", 800, 600);
	cdeltat->SetLogx();
	cdeltat->SetLogy();
	hdeltat->Draw();
...
/*
// get B class triggers and draw them on A class triggers

TFile flon("hlon_off.root")
TCanvas *c = (TCanvas *) flon.Get("c2_7")
TH1D *hl =(TH1D *) c->GetPrimitive("htemp")
hl->Draw() 
*/
