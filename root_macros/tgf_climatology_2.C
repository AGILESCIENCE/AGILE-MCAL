...
// 	mcalstyle->SetLabelSize(0.05, "Z");
	...
	TCanvas *c11 = new TCanvas("c11", "c11", 1200,400);	//1000,300
	//c11->Divide(1,2);
	//c11->cd(1);
	...
	TH2F *htmp = new TH2F("htmp", "htmp", 360, -180.5, 179.5, 41, -20.5, 20.5); // 60, -3., 3.
	...
	hrfc->GetYaxis()->SetRangeUser(-20., 20.);
	hrfc->GetYaxis()->SetTitleOffset(0.8);
	hrfc->GetZaxis()->SetTitle("Flashes / km^{2} year");
	hrfc->GetZaxis()->SetTitleSize(0.06);
	hrfc->GetZaxis()->SetLabelSize(0.06);
	hrfc->GetZaxis()->SetTitleOffset(0.7);
	hrfc->GetZaxis()->CenterTitle();
	// hrfc->SetContour(2);
	hrfc->Draw("COLZ");
	htmp->SetContour(1);
	htmp->SetLineWidth(2);
	...
	for (int i=0; i<nentries; i++) {
		...
		int imonth = (year - 2009)*12 + month;
		if (imonth < 3 || imonth > 14) continue;	// use only data from Mar.09 to Feb.10 (like paper FF)
		...
		TMarker *m = new TMarker(atrg.lon, atrg.lat, 23); //20, 8
		
		// scale dimension according to fluence
		float dim =  0.3 + 0.05*atrg.counts;  //0.08
		m->SetMarkerSize(dim);
	...
	*/		
		// color version
		if (hr < 0.5) color = 1;
		else if (hr < 1.) color = 1;
		else if (hr < 2.) color = 6;
		else if (hr < 3.) color = 7;
		else if (hr < 4.) color = 7;
		else color = 7;
