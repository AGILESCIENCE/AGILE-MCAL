int burst_search_3(char *inpath="./tmp.root", char *outpath="./output.dat")
{
	...
	// get time and energy arrays, keep ONLY events with energy above threshold
	int nevt = 0;
	for (int i=0; i<nevents; i++) {
		data->GetEntry(i);
		if (inevt.Etot >= ethr) {
			time[nevt]   = inevt.time;
			energy[nevt] = inevt.Etot;	
			nevt++;
		}
	}
	...
	for (int i=0; i<nevt-thr; i++) {	
		...
		if (time[i+thr-1] > time[i] && time[i+thr-1] - time[i] <= dt) {
			...
			printf("%6d  %10.6f  %10.3f %10.6f  %10.3f %10.6f  %10.3f  ", i, time[i], energy[i], time[i+1]-time[i], energy[i+1], time[i+2]-time[i], energy[i+2]);	// debug!
			n = nle = nhe = 0;
			emax = eavg = 0.;
			...
			while (time[i+n+1]-time[i+n] <= dt2) {	// modifica rispetto alla logica subms standard
				...
				// CANNOT WORK WITH REDUCED DATASET
/*				data->GetEntry(i+n);
				for (int k=0; k<30; k++) {
					int id = inevt.bar[k];
					if (id>=0) {
