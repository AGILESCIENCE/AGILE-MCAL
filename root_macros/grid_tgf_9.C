int grid_tgf_9(int contact =7467, double t0=150112496.460176, double deltat=0.002, double lst2=10.747369)
{
	// MM 20/07/10 get in input also LST calculated with IDL. Is this the cause of E distance discrepancy?
	// 	       also routine ProjectVector() gets lst as input and bypass approximated calculation
	//             based on grid_tgf_7.C
	...
	char evtfile[200];
	GRIDevt gevt;
	sprintf(evtfile, "/home/mcal/data_analysis/VSB/grid/data/EVTP_reversed/evt_merged.fits.root", contact);
	...
	while (time[n] <= t0+deltat) {
		...
		if (time[n] >= t0) {
			...
			projectVector(gevt.RA, gevt.Dec, levt0.x, levt0.y, levt0.z, levt0.time, lst2);
...
projectVector(double ra = 106.6149, double dec = -43.99318, double x0 = 6717.70960710422, double y0 = 1632.85447954786, double z0 = -147.114275690763, double time = 159242835.342130, double lst3=10.747369) 	// default evt @ contact 8958
{
	...
	double lst = lst3*360./24.; 	// get lst as input, bypass approximated calculation
