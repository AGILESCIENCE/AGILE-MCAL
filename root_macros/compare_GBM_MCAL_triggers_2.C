{
	// MM 03/01/14 update check for period 28/01/13 - 31/12/13. 
	// Changed readout of IPN master list; modified limits on t0
...
//	fgbm.open("/home/mcal/data_analysis/grb/IPN-supplement-GBM-catalog/gbmcat.txt", ifstream::in);
	fgbm.open("/home/mcal/data_analysis/grb/IPN_catalog/IPN_master_list_check_140103/gbmcat_2013.txt", ifstream::in);
	int ngbm = 0;
	int tthr = 10.;
	...
	while (fgbm >> grbname >> grbdate >> grbtime) {
		...
		if ((t0gbm.GetSec() - t03908[pos] <= tthr) || (t03908[pos+1] - t0gbm.GetSec() <= tthr)) {
			...
			if ((t0gbm.GetSec() - ttipn[ipnpos] <= tthr) || (ipnpos<nipn-1 && ttipn[ipnpos+1] - t0gbm.GetSec() <= tthr)) {
				inlist = 1;
				// printf("%4d  %12d  %12d  %12d\n", ipnpos, t0gbm.GetSec(), ttipn[ipnpos], ttipn[ipnpos+1]);  
			} else inlist = 0;
		}
