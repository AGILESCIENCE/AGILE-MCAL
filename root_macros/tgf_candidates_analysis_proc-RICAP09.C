...
// 	mcalstyle->SetLabelSize(0.05, "Z");
	...
	TPad *p1 = new TPad("p1", "p1", 0.05, 0.05, 0.75, 0.95);
	TPad *p2 = new TPad("p2", "p2", 0.80, 0.05, 0.95, 0.95);
	p1->Draw();
	p2->Draw();
	p1->cd();
	...
	TMarker *m1 = new TMarker(0.1, 0.9, 20); 
	m1->SetMarkerSize(2.);
	m1->SetMarkerColor(1);
	m1->Draw();
	t->DrawTextNDC(0.3, 0.88, "0.5 < HR < 1.");
	
	TMarker *m2 = new TMarker(0.1, 0.8, 20); 
	m2->SetMarkerSize(2.);
	m2->SetMarkerColor(15);
	m2->Draw();
	t->DrawTextNDC(0.3, 0.78, "1.  < HR < 2.");
	
	TMarker *m3 = new TMarker(0.1, 0.7, 20); 
	m3->SetMarkerSize(2.);
	m3->SetMarkerColor(16);
	m3->Draw();
	t->DrawTextNDC(0.3, 0.68, "2.  < HR < 3.");
	
	TMarker *m4 = new TMarker(0.1, 0.6, 20); 
	m4->SetMarkerSize(2.);
	m4->SetMarkerColor(17);
	m4->Draw();
	t->DrawTextNDC(0.3, 0.58, "3.  < HR < 4.");
	
	TMarker *m5 = new TMarker(0.1, 0.5, 20); 
	m5->SetMarkerSize(2.);
	m5->SetMarkerColor(18);
	m5->Draw();
	t->DrawTextNDC(0.3, 0.48, "       HR > 4.");
	
	TMarker *m6 = new TMarker(0.15, 0.4, 24); 
	m6->SetMarkerSize(1.);
	m6->Draw();
	t->DrawTextNDC(0.3, 0.38, "10 counts");
	
	TMarker *m7 = new TMarker(0.15, 0.3, 24); 
	m7->SetMarkerSize(2.);
	m7->Draw();
	t->DrawTextNDC(0.3, 0.28, "20 counts");
	
	TMarker *m8 = new TMarker(0.15, 0.2, 24); 
	m8->SetMarkerSize(3.);
	m8->Draw();
	t->DrawTextNDC(0.3, 0.18, "30 counts");
