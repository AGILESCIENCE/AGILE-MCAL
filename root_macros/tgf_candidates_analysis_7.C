...
// 	mcalstyle->SetLabelSize(0.05, "Z");
	...
	Trigger atrg;
	 ...
	 // tvsb.ReadFile("/data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_Mar2009-Feb2011.txt", 
	  //tvsb.ReadFile("/data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_Mar09-Feb11_E-greater-30MeV.txt",
	   //tvsb.ReadFile("/data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_Mar2009-Feb2011_E-greater-30MeV.txt", 
	   //tvsb.ReadFile("/data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_Mar2009-Feb2011.txt",
	   tvsb.ReadFile("/data1/mcal/data_analysis/VSB/tgf_candidates_complete/green_candidates_ALL.txt",
	...
	int lonbin = 36; // 12 o 36 o 72 o 144
	...
	*/		
		...
		if (1) { // year>=2009 && month>=3
			...
			// 27/07/2011 - format for UT timing check 
			if (0) {
				//printf("%6d \t%20.6f \t%8.6f\n", atrg.runid, atrg.t0, atrg.t90, atrg.dt0, atrg.tgrbini);
				ts.Set((int)  atrg.t0, 0, epoch.GetSec(),0);
				double peakdt = (fmod(atrg.t0,1.) + atrg.tgrbini);
				if (peakdt>=0. && peakdt<1.) {
					printf("%06d \t%20.6f \t%s\n",atrg.runid, atrg.t0 + atrg.tgrbini, ts.AsString("s"));
				} else cout << "check trigger " << atrg.runid << "-" << atrg.trg << endl;
			}
