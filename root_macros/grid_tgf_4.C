int grid_tgf_4(int contact =7249, double t0=148782321.371584, double deltat=0.002)
{
	// MM 17/05/10 work on events with 0 < t-t0 < 2ms, dump everything for check purpose
...
{
	int verbose = 1;	// if 1: print debug info, if 0: print output
	...
	if (verbose) cout << "LST = " << lst << "  "  << lst/360.  << "  "   << fmod(lst, 360.) << " deg " << endl;
	lst = fmod(lst, 360.);
	...
	if (discr2 > 0.) {
		...
		} else printf(" \t%8.3f \t%8.3f \t%8.3f", angle*rad2deg, lindist, arcdist);
