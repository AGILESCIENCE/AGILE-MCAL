{
	// MM 14/01/10 small changes to procuce pictures for 7th AGILE WS proceedings
...
// 	mcalstyle->SetLabelSize(0.05, "Z");
	...
	for (int i=0; i<nentries; i++) {
		...
		if (year == 2009 && month>=3 && month<=11) hm->Fill(month);
...
/*	c1.cd(1);
	...
	TCanvas c4("c4", "c4", 1200, 400);//, 800, 600
	c4.Divide(3, 1);	// 2,2
	c4.cd(1);
	//gPad->SetLogx();
	//gPad->SetLogy();
...
/*	c4.cd(3);
	...
	hcpeak->GetYaxis()->SetTitle("Events");
	hcpeak->Draw();
...
*/	
	...
	havgE->GetYaxis()->SetTitle("Events");
	havgE->Draw();
