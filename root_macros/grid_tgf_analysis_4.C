...
// 		htmp3->Fill(x,y, 1);
	...
	ifstream in;
	in.open("/home/mcal/data_analysis/VSB/grid/grid_results_proj.dat");	// vedi README
	grecord gevt;
	GRIDevt eevt;
	char aux[20];
	int  iaux;
	while (in >> aux >> gevt.contact >> gevt.t0 >> gevt.ievt >> gevt.t >> gevt.dt >> iaux >> eevt.energy >> eevt.theta >> eevt.phi >> eevt.RA >> eevt.Dec >> eevt.ph_earth >> eevt.brname03 >> eevt.evstatus >> eevt.lon >> eevt.lat >> eevt.k >> eevt.lon_P >> eevt.lat_P >> eevt.alpha >> eevt.dist_L >> eevt.dist_P >> eevt.distE >> eevt.distN) {
