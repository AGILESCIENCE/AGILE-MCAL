...
TDatime dstart(2007, 1, 29, 0, 0, 0);
TDatime dstop(2007, 2, 6, 24, 0, 0);
...
TFile *f= new TFile("/data1/mcalsw_output/misc/MCAL_global_data_IABG_2007_TV.root");
...
XA->GetXaxis()->SetTimeOffset(da.Convert());
...
ZA->GetXaxis()->SetTimeOffset(da.Convert());
...
XB->GetXaxis()->SetTimeOffset(da.Convert());
...
ZB->GetXaxis()->SetTimeOffset(da.Convert());
...
TH1F *hg;
	...
	TCanvas *cx = new TCanvas("cx", "Sci. RM 2 (170 - 350 keV)", 800, 600);
	cx->Divide(1,2);
	for (int i=1; i<=11; i++) {
		// work on Sci RM X
		aux2 = NULL;
		...
		if (aux2) {
			...
			aux2->GetXaxis()->SetTimeOffset(da.Convert());
		...
		hs += i;
			...
			//hg = new TH1F(hname, hname, (int) (tend-tini), tini, tend);
			hg = new TH1F(hs.Data(), hs.Data(), (int) (tend-tini), tini, tend);
			npt = aux2->GetN();
			x = aux2->GetX();
			y = aux2->GetY();
...
// plot run id
	...
	for (int i=0; i<nrun; i++) {
		...
		if (info->tstart>tini && info->tstart<tend) {
			...
			t = new TText(info->tstart+100., 200., srun.Data());    // NB: TERMINE -3600 COME SOPRA
			t->SetTextAngle(90.);
			t->SetTextColor(2);			
			t->Draw();
...
TDatime dstop(2007, 2, 6, 24, 0, 0);
 ...
 TString dataname("/data1/mcalsw_output/misc/MCAL_global_data_IABG_2007_TV.root");
 TString hkname("/data1/AGILE_MCAL_AIV_Data/IABG/delta_qualifica_gennaio07/TV/hk_tv_iabg2007.root");
long int ti = da.Convert();
 float  tini=dstart.Convert() - da.Convert();
 float  tend=dstop.Convert() - da.Convert();
