int plot_triggers_9_no_E_cut(char *inpath="./tmp.root", char *outpath="./output.root", char *outpath2="./selection.dat")
{
	// MM 22/07/2016 removes cut on E max (to answer to Dwyer request on email 11/07/16)

	// MM 04/11/2015 creates also an additional txt output file with only selection on Central America for correlation with Steve Cummer's data
	// MM 23/10/2015 removed selection on the presence of trigger (need to fix trigger timing first). workaround for script tgf4.py

	// MM 28/07/2015 removed selection on boost (should be obsolete since last threshold change), after PPSE failure on Jul 1st 2015. 
	// based on macro plot_triggers_7.C, to be used within scripl tgf3.py
...
//	TFile *newfile = new TFile("/home/mcal/data_analysis/VSB/offline_trigger_search/trigger_search_2/LE_TGF_Mar09-17Feb15.root","recreate");
	...
	// write relaxed selection on Central America for correlation studies with Steve Cummer
	...
	while (ttrg.GetEntry(i++)) { // NB: check the selection on max energy (E_max <= or > 30 MeV) 
		...
		if (atrg.lon>=-90. && atrg.lon<=-50.) fprintf(fo, "%d \t%02d \t%02d \t%02d \t%02d \t%02d.%06d \t%8.3f \t%8.3f \t%.3f \t%06d \t%.6f\n", atrg.year, atrg.month, atrg.day, atrg.hour, atrg.min, atrg.sec, atrg.usec, atrg.lon, atrg.lat, atrg.h*.001, atrg.contact, atrg.t0);
	
		if (atrg.counts>9 && atrg.hr>=0.5 && (atrg.c[0] + atrg.c[2]) > 0 && (atrg.c[1] + atrg.c[3]) > 0 && (atrg.c[4] + atrg.c[6]) > 0 && (atrg.c[5] + atrg.c[7]) > 0) {
		
			nt++;
			printf("%6d \t%3d \t%20.6f\n", atrg.contact, atrg.id, atrg.t0); // list for timing analysis and geographyc info calculation
			newtree->Fill();
		}
	}
	cout << nt << endl;
