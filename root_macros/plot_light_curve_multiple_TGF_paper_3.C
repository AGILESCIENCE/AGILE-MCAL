{
	// MM 21/09/12 plot multiple light curves for multi-peaked TGFs in catalog #1 (Mar.09 - Feb.12)
	// based on plot_light_curve_multiple_TGF_paper_2.C
	...
	char *trg[] = {"TGF 090309.37239", "TGF 091011.64173", "TGF 091106.90098", "TGF 100612.58905", "TGF 111024.60597", "TGF 111103.86414", "TGF 111122.06618"};
	
	// input data
	
	double atbin = 0.0002;
	double azbefore = -0.005; //-0.0048;
	double azafter  =  0.010; //0.0099;
	double atbkgini = -2.9;
	double atbkgfin  =  0.0;
	for (int i=0; i<20; i++) {
		tbin[i]    = atbin;
	...
	// 
	rootfilename[0] = new TString("/MCAL_PROC/ARCHIVE/009682/RT009682_3908.root");
	t0[0] =  163673775.205678; // tgrbini = 0.0096
	
	// 
	rootfilename[1] = new TString("/MCAL_PROC/ARCHIVE/012735/RT012735_3908.root");
	t0[1] =  182359446.361012; // tgrbini = 0.0036
	
	// 
	rootfilename[2] = new TString("/MCAL_PROC/ARCHIVE/013106/RT013106_3908.root");
	t0[2] = 184628245.077404; // tgrbini = 0.0126
	
	// 
	rootfilename[3] = new TString("/MCAL_PROC/ARCHIVE/016180/RT016180_3908.root");
	t0[3] = 203436494.135051; // tgrbini = 0.0044
	
	// 
	rootfilename[4] = new TString("/MCAL_PROC/ARCHIVE/023229/RT023229_3908.root");
	t0[4] = 246551556.095935; 
	
	// 
	rootfilename[5] = new TString("/MCAL_PROC/ARCHIVE/023374/RT023374_3908.root");
	t0[5] = 247437862.550726;
	
	// 
	rootfilename[6] = new TString("/MCAL_PROC/ARCHIVE/023631/RT023631_3908.root");
	t0[6] = 249010518.595219; // 
...
*/
	...
	double label_size   = 0.1;	// 0.07
	double text_size    = 0.1;	// 0.07
	double title_offset = 0.35;	// 0.6 ma occorre sistemare i boundary dei plot di dx, che a mano non si riesce!
	double xpos = 0.15;
	double zpos = 0.7;
	...
	for (int i=0; i<7; i++) {
		TString pname("pad_");
		pname += i*2;
		apad[i*2]  = new TPad(pname, pname, 0.001, fa + fr*(6-i)/7. + 0.001, 0.499, fa + fr*(7-i)/7. - 0.001);	// sx pads
		apad[i*2]->Draw();
		TString pname1("pad_");
		pname1 += i*2+1;
		apad[i*2+1] = new TPad(pname1, pname1, 0.501, fa + fr*(6-i)/7. + 0.001, 0.999, fa + fr*(7-i)/7. - 0.001);	// dx pads
		apad[i*2+1]->Draw();
	}	
	apad[14]  = new TPad("plabsx", "plabsx", 0.01, 0.001, 0.49, fa-0.001);
	apad[14]->Draw();
	apad[15]  = new TPad("plabdx", "plabdx", 0.51, 0.001, 0.99, fa-0.001);
	apad[15]->Draw();
	...
	//c1.Print("/home/mcal/data_analysis/VSB/offline_trigger_search/trigger_search_2/images/multiple.eps");
