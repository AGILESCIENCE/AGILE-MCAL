...
// 	mcalstyle->SetLabelSize(0.05, "Z");
	...
	while (!frhessi.eof()) {
		...
		if (rtgf[nrh].lat<=2.5 && rtgf[nrh].lat>=-2.5) {
			hlonRH->Fill(rtgf[nrh].lon);
			hlcRH->Fill(rtgf[nrh].loctime);
			cout << nsel << "  " << rtgf[nrh].date << "  "  << rtgf[nrh].lat << "  " << rtgf[nrh].lon << endl;
			nsel++;
		}
