{
	// MM 28/07/2015 version 4. removes checks on BOOST flag, to be used after PPSE failure on Jul 1st 2015 	
...
// 	mcalstyle->SetLabelSize(0.05, "Z");
	...
	TH1I *htrg[nh];
	int ci=43398;//42682
	int cf=43599;//25850 // upper edge excluded!
	int nbins=200;
	for (int i=0; i<nh; i++) {
		TString hname("h");
		hname += i;
	...
	ifstream fin;
	fin.open("/AGILE_PROC3/DATA/HLT/MCALBURST/mcal_grb.dat", ifstream::in); //mcal_grb.dat.archive
	int ntrg = 0;
	while (!fin.eof()) {
		...
		if (atrg.runid>=ci && atrg.runid<=cf ) {	// atrg.type[0]=='G' the trigger is flagged GRB
			...
			if (atrg.nrm > 1 && atrg.sa == 0 ) { // SW trg classified as GRB
			...
			int ibin = (int) ((atrg.runid - ci)*nbins/(cf-ci) + 1);
			httot->AddBinContent(ibin, atrg.ttot);
	...
	htrg[5]->Draw();
        ...
        TCanvas c5("long_triggers", "long_triggers", 800, 600);
        htrg[6]->SetStats(0);
        htrg[6]->Draw();
