...
// 	mcalstyle->SetLabelSize(0.05, "Z");
	...
	for (int i=0; i<nentries; i++) {
		...
		if (atrg.t0 != t0prev && atrg.mint<-25. && atrg.ntrg>0 && (atrg.runid<7302 || atrg.runid>7342) && (atrg.runid<6068 || atrg.runid>6140) ) {
			...
			hlontot->Fill(atrg.lon<=180. ? atrg.lon : atrg.lon-360.);
			hdttot->Fill(atrg.dt);					
			...
			if (fabs(atrg.tgrbini)<0.060 && atrg.counts>0  ) {
				...
				if (atrg.type[0]=='G' ) {
					...
					hlon->Fill(atrg.lon<=180. ? atrg.lon : atrg.lon-360.);
					hdt->Fill(atrg.dt);	
					hhr->Fill(hr);
				...
				hdtlon->Fill(atrg.lon<=180. ? atrg.lon : atrg.lon-360., atrg.dt);		
					...
					// 
					// if (hr>0.5) cout << atrg.runid << "  " << atrg.trg << "  " << atrg.counts << "  " << atrg.cpeak << "  " << atrg.subms << "  " << hr << "  " << atrg.dt << endl;
				}
				...
				if (atrg.type[0]=='B') {
					...
					hlonBUS->Fill(atrg.lon<=180. ? atrg.lon : atrg.lon-360.);
					hdtBUS->Fill(atrg.dt);					
					hhrBUS->Fill(hr);
