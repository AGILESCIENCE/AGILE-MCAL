{
	// MM 03/04/09 limited modifications in output printing, for Feb-Mar09 data analysis
...
// 	mcalstyle->SetLabelSize(0.05, "Z");
	...
	for (int i=0; i<nentries; i++) {
		...
		if (atrg.t0 != t0prev  ) {
			...
			if (atrg.counts>9 && hr>0.2) { // && atrg.counts>6 && hr>0.2 && fabs(atrg.tgrbini)<0.020
				
				// format for sferics correlation
				double peakdt = (fmod(atrg.t0,1.) + atrg.tgrbini);
				if (0 && peakdt>=0. && peakdt<1.) {
				...
				if (0) printf("%s \t%s \t%6d \t%3d \t%s \t%6.2f \t%6.2f \t%3d \t%3d \t%3d \t%6.3f  \t%8.2f \t%6.2f \t%6.1f \t%20.6f \t%8.6f\n", atrg.date, atrg.time, atrg.runid, atrg.trg, atrg.type, atrg.counts + atrg.cbkg, atrg.cpeak, atrg.subms, atrg.flg01ms, atrg.flg16ms, hr, atrg.lon, atrg.lat, atrg.dt, atrg.t0, atrg.t90);
				...
				hdtlon->Fill(atrg.lon, atrg.dt);		
					...
					// format for checking with Fullekrug results
					if (1) printf("%s \t%s \t%6d \t%3d \t%6.2f \t%6.2f \t%3d \t%3d \t%3d \t%6.3f  \t%8.2f \t%6.2f \t%6.1f \t%20.6f \t%8.6f \t%8.6f\n", atrg.date, atrg.time, atrg.runid, atrg.trg, atrg.counts + atrg.cbkg, atrg.cpeak, atrg.subms, atrg.flg01ms, atrg.flg16ms, hr, atrg.lon, atrg.lat, atrg.dt, atrg.t0, atrg.t90, atrg.tgrbini);
				}
