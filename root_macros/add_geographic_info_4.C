#include <string.h>
int add_geographic_info_4(char *inpath="./tmp.root", char *outpath="./output.dat")
{
	// MM 22/10/2015 changed gpsevt structure according to the changes made at class MCVFile3916
	// MM 08/04/2015 input and output path as external parameters. based on add_geographyc_info_3.C
	...
	inFile.open(inpath);	// on HP1, update to 31/03/2015
	...
	while (inFile >> aux >> path >> trgid >> t0 >> counts >> hr >> dt >> eavg >> emax >> c[0] >> c[1] >> c[2] >> c[3] >> c[4] >> c[5] >> c[6] >> c[7]) {
		evt.time = (Double_t) t0;
		printf("*** evt file %s  trg id %d  t0 %20.6f\n", path, trgid, evt.time);
		evt.gpsxpos = evt.gpsypos = evt.gpszpos = evt.posvali = evt.lon = evt.lat = evt.etheta = evt.ephi = evt.h = 0.;
		//evt.posvali = 0.;
		//sscanf(path, "/MCAL_PROC/ARCHIVE/%06d", &contact);
		sscanf(path, "/home/agileuser/MCAL_PROC/ARCHIVE/%06d", &contact); // MM 09/12/15 for analysis done on ASDC agilehp4
		//sscanf(path, "/home/mcal/data_analysis/MCAL_PROC2/ARCHIVE/%06d", &contact); // MM 21/10/15 local path to avoid problem in 3908 archive
		...
		sprintf(infile, "/home/mcal/data_analysis/AGILE_coordinates/RT%06d_3916.root", contact);
                MCVFile3916 *f = new MCVFile3916(infile, 1); // verbose
