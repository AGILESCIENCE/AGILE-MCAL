int burst_search_5(int inicont, int fincont) 
{
	// MM 14/04/14 to be used for real-time TGF analysis
	// based on burst_search_2.C, add_geographic_info_3.C, plot_triggers_4.C and plot_triggers_5.C (used for 1st catalog)
	// work on a range of contacts

	// MM 15/10/12 corrected duration calculation at line 117. must be n-1 instead of n!
	...
	double dt2 = 0.0003;	// max time separation to define a cluster of events
	int thr   = 6;	// 9	// events threshold in dt1

	for (int cont=inicont; cont<=fincont; cont++) { // loop on the number of contacts
		// load root events file
		sprintf(inpath, "/MCAL_PROC/ARCHIVE/%06d/RT%06d_3908.root", cont, cont);
		fi= new TFile(inpath);
		if (fi->IsZombie()) { 	// fi is not a valid root file: exit
			cout << inpath << " is not a valid ROOT file: moving on!" << endl;
			continue;
		...
		for (int i=0; i<nevents-thr; i++) {	
			...
			// this part is from burst_search_2.C
