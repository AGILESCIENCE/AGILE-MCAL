...
void MCVFilePKP::Process3909Data()
{
	...
	pktnumb = new long[nevents]; //unsigned
	evtnumb = new Short_t[nevents];
	sec = new long[nevents];
	micsec = new long[nevents];
...
<<<<<<< MCVFilePKP.cpp
	//if (fits_read_col(fptr, TINT32BIT, 4, 1, 1, nevents, 0, micsec, NULL, &status)) printerror(status); // MM 20/10/15 commented
=======
