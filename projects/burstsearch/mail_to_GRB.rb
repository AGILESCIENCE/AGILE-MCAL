...
#	$3	ORBIT
	...
	puts("\nIncorrect number of parameters","for mail_to_GRB.rb",ARGV,"","Cannot verify the condition","to send the mail","");
	mail_obj = ""
else
	Dir.chdir(ARGV[0])
			...
			File.open(file_name).each_line do | line |
				...
				if word[0] == nil 
					...
					File.open(dir1[0]).each_line do | time_tt |
						puts "generating MCAL light curve"
						...
						alert_grid=String.new("TOMAIL:   fuschino@iasfbo.inaf.it  \nALERT_TYPE:   MCAL 3913 ground trigger  \nALERT_TYPE_CODE:   20  \nBURST_START:   "+time_tt.to_s.chop+"\nORBIT_NUM:   "+ARGV[2] );
                                                ...
                                                mail_GRID_send="nail -s 'MCAL trigger @ orbit "+ARGV[2].to_s+"'  -A mcal -c fuschino@iasfbo.inaf.it agile_grid_burst@iasfbo.inaf.it delmonte@iasf-roma.inaf.it < "+alert_path
#                                               mail_GRID_send=String.new("nail -s 'MCAL trigger @ orbit "+ARGV[2].to_s+"'  -A mcal fuschino@iasfbo.inaf.it < "+alert_path);
					...
					send_mail=String.new("nail -s '" + mail_obj.chop + "' -a " + dir2[0].to_s + " -A mcal mcal@iasfbo.inaf.it  delmonte@iasf-roma.inaf.it < " + dir0[i])
					system send_mail
					puts "\n\nMail sent for GRB detection with following Object: \n" + mail_obj.chop + "\nand with " + dir2[0].to_s + "   in attach \n\n"
					puts dir2
					...
					mail_obj=String.new(line.to_s)
					system "nail -s '"+mail_obj+"' -A mcal mcal@iasfbo.inaf.it < "+dir0[i]
					puts "\nMail sent for GRB detection with following Object: \n\n"+mail_obj+"\n\n"
					if word[1] == "SOE" then
                         			mail_obj=String.new(line.to_s)
