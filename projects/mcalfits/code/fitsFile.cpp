...
void  fitsFile::setStringKey(std::string keyname, char *value, std::string hduname) 
{
    int status_fits=0 ;
    char nome[80] ;
    strncpy(nome,hduname.c_str(),80) ; 
    ...
    {
       errorefits(status_fits,0,
                   0," setStringKey: hdu moving ") ; 
    }   
    setStringKey( keyname,  value ) ; 
}
void  fitsFile::setStringKey(std::string keyname, char *value)
{
    int status_fits=0 ; 
    char nome[80] ;
    strncpy(nome,keyname.c_str(),80) ; 

    fits_update_key(fitsPointer,TSTRING,nome,value,NULL,&status_fits);
    if(status_fits)   
    {
   	errorefits(status_fits,0,0," Routine setStringKey ") ; 
    }    
}
