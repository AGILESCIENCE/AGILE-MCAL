...
if ARGV.size == 2  then
	...
	if FileTest.file?(ARGV[0]) then
		File.open(ARGV[0]).each_line do | fileline | 
			...
			if a[7]=="GRB"  &&  a[10].to_i==0  &&  a[11].to_i==1  &&  a[13].to_i==0 then
				...
				#do VSB analysis
				system "plotgrb -r "+root_filename+" -t0 "+a[2]+" -3 3 -tbin 0.0001 -batch -o "+outpath+" -diag -save -subms 0.001 10"
